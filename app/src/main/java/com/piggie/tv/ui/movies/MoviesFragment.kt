...
class MoviesFragment : Fragment() {
    ...
    private fun loadData() {
        ...
        val context = requireContext()
        val loadingLabel = label("Loading Movies...", 18f, R.color.tv_text_secondary, margin = 20, leftPadding = context.dim(R.dimen.tv_screen_margin_horizontal))
        page.addView(loadingLabel)

        thread(start = true) {
            runCatching {
                api.loadMoviesIncrementally(session) { shelf ->
                ...
                activity?.runOnUiThread {
                    page.addView(label("All Movies", context.dimFloat(R.dimen.tv_text_size_section_title), R.color.tv_text_primary, true, margin = 32, leftPadding = context.dim(R.dimen.tv_screen_margin_horizontal), isPx = true))
                    setupGrid()
                    loadGrid()
                }
            }.onSuccess {
    ...
    private fun setupGrid() {
        ...
        val recycler = RecyclerView(context).apply {
            ...
            gridAdapter = MovieGridAdapter(movies, api, session) { position ->
                if (position >= movies.size - 10 && !gridLoading && hasMore) {
                    loadGrid()
                }
            }
            adapter = gridAdapter
    ...
    private fun loadGrid() {
        ...
        thread(start = true) {
            runCatching {
                api.loadMovies(session, startIndex, limit)
            }.onSuccess { results ->
                activity?.runOnUiThread {
                    if (results.isEmpty()) {
                        hasMore = false
    ...
    private class MovieGridAdapter(
        private val items: List<MediaItem>,
        private val api: JellyfinNativeApi,
        private val session: NativeSession,
        private val onBind: (Int) -> Unit
    ) : RecyclerView.Adapter<MovieGridAdapter.Holder>() {
        override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): Holder {
            val context = parent.context
            val card = LinearLayout(context).apply {
                orientation = LinearLayout.VERTICAL
